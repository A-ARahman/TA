...
?>
<main class="edit">
<form name="fdata_stedit" id="fdata_stedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["data_id", [fields.data_id.visible && fields.data_id.required ? ew.Validators.required(fields.data_id.caption) : null], fields.data_id.isInvalid],
...
<?php if ($Page->data_id->Visible) { // data_id ?>
    <div id="r_data_id"<?= $Page->data_id->rowAttributes() ?>>
        <label id="elh_data_st_data_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->data_id->caption() ?><?= $Page->data_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->data_id->cellAttributes() ?>>
<span id="el_data_st_data_id">
<span<?= $Page->data_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->data_id->getDisplayValue($Page->data_id->EditValue))) ?>"></span>
<input type="hidden" data-table="data_st" data-field="x_data_id" data-hidden="1" name="x_data_id" id="x_data_id" value="<?= HtmlEncode($Page->data_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fdata_stedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdata_stedit", "x_timestamp", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdata_stedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdata_stedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
